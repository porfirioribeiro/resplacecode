...
class dbList extends Module {
	...
	function content(){
		...
		if ($arr){
			foreach ($arr as $file) {
				?>
				<a href="javascript:void(0)" onclick="if (confirm('Delete this Database?\n<?=$file?>')){document.location='?manage=db&action=delDB&place=data&db=<?=$file?>'}"><img alt="Delete" title="Delete this Database" border="0" style="vertical-align:middle" src="icons/button_cancel.png"></a>&nbsp;<a href="?manage=db&action=editdb&place=data&db=<?=$file?>"><?=$file?></a><br>
				<?php
			}			
		}
		?>
