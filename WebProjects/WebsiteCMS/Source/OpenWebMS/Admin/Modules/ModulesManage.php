...
class ModulesManage extends Module {
	...
	function content(){
		...
		if (isset($_POST['editpage']))
			...
			WriteFile($WebMS['UserModulesPath'].$_POST['edit'].".php",stripslashes($_POST['data']));
			?></div><br><br><?php
			}
		...
		if (isset($WebMS['URLArray'][2]) && ($WebMS['URLArray'][2]=="Del"))
			...
			Request should have succeeded.
			<?php
			unlink($WebMS['UserModulesPath'].$WebMS['URLArray'][3].".php");
			?></div><br><br><?php
			}
		...
		<?php
		$files=GetFiles($WebMS['UserModulesPath']);
		if (count($files)) {
			foreach ($files as $fil) {
				...
				if (isset($name[1]) && $name[1]=='php') {
					...
					echo'<tr>
								<td class="sub"><a href="'.url(array("*","*","Edit",$name[0])).'"><img alt="Edit" title="Edit this module" border="0" style="vertical-align:middle" src="'.$WebMS['AdminUrl'].'icons/edit.png"></a></td>
								<td class="sub"><a href="javascript:void(0)" onclick="if (confirm(\'You sure you want to delete this module?\n'.$name[0].'\')){document.location=\''.url(Array("*","*","Del",$name[0])).'\'}"><img alt="Delete" title="Delete this module" border="0" style="vertical-align:middle" src="'.$WebMS['AdminUrl'].'icons/button_cancel.png"></a></td>
		...
		?>
		<tr><td colspan="3" class="sub"><a href="<?=url(array("*","*","Add")); ?>">Add new module</a></td></tr>
		</table>
		<br>
		</div><br><br>
		<?php
		...
		if (isset($WebMS['URLArray'][2]) && ($WebMS['URLArray'][2]=="Edit"))
			...
			$file=$WebMS['UserModulesPath'].$WebMS['URLArray'][3].".php";
			$fh=fopen($file,'r');
