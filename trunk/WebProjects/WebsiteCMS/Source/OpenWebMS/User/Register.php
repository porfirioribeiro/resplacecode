...
function someContent($mod){
	...
	if (isset($_REQUEST["activate"])) {
		if (isset($_REQUEST["username"])) {
			...
			print_r($result);
			$mail=$result["0"]["activatestring"];
			if ($_REQUEST["activate"]==$mail) {
				$result=$db->query("UPDATE ".$WebMS["MySQL_Prefix"]."users SET activate='1', activatestring='' WHERE usrname='".$usrname."'",false);
			} else {
				echo $_REQUEST["activate"].'=='.$mail;
			}
		}
	} else {
			...
			if ((strcmp($_POST["password"],$_POST["password2"])==0)) {
				...
				if (strlen($_POST["password"])>=5) {
					...
					if (check_email($_POST['mail'])) {
						...
						if ($db->query("INSERT INTO ".$WebMS["MySQL_Prefix"]."users (usrname,psswrd,email,name,activate,activatestring) VALUES('".$_POST['username']."','".$_POST['password']."','".$_POST['mail']."','".$_POST['username']."','0','".md5($_POST['mail'])."')",false)) {
